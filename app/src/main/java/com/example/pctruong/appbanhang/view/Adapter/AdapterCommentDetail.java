...
/**
 * Created by PCTruong on 08/06/2018.
 *
...
public class AdapterCommentDetail extends RecyclerView.Adapter<AdapterCommentDetail.ViewHolder> {
    Context context;
    List<BinhLuan> binhLuanList;

    public AdapterCommentDetail(Context context, List<BinhLuan> binhLuanList) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.txtNoidung.post(new Runnable() {
            @Override
            public void run() {
                int count=holder.txtNoidung.getLineCount();
                if(count>5){
                    holder.txtNoidung.setLines(5);
                }
            }
        });
    }
